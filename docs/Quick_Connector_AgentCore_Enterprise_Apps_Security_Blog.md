...
本文以“查询订单需求，再检查库存可用性”为例，介绍通过 Amazon Quick MCP 连接器和 Amazon Bedrock AgentCore Gateway 安全访问企业应用的四种方案：服务身份访问、混合下游认证、用户身份委托，以及私网访问。四个场景使用相同的只读业务逻辑，分别展示不同的身份、凭据和网络边界。
...
本文将 Amazon Quick 简称为 **Quick**，将 Amazon Quick MCP 连接器简称为 **连接器**，将 Amazon Bedrock AgentCore Gateway 简称为 **Gateway**，将 Microsoft Entra ID 简称为 **Entra ID**。MCP 指模型上下文协议（Model Context Protocol）；Gateway target 指 Gateway 接入的业务函数或 API。
...
| 场景二 | Cognito client credentials flow | 订单：Basic 认证；库存：Entra ID client credentials flow | 订单 REST API；库存 HTTP API | 同时接入传统系统和使用企业应用身份的 API |
| 场景三 | Entra ID authorization code flow | Entra ID 代表用户流程（On-Behalf-Of，OBO） | 两个 HTTP API | 根据当前用户的业务角色授权 |
| 场景四 | Cognito client credentials；resource-server走Quick VPC连接 | IAM | 专属VPC Lambda target | Gateway路径PrivateLink，业务函数运行在VPC内 |
...
*图 3：场景一的 Cognito 用户池配置。*

![场景一的 Cognito app client 配置](image/image-20260913212144736.png)

*图 4：场景一的 Cognito app client 配置。*

**Gateway execution role 与入站授权。** execution role 仅能调用两个业务函数，Gateway 使用 `CUSTOM_JWT` 入站授权。

![场景一的 Gateway execution role 权限](image/image-20260911115543751.png)

*图 5：场景一的 Gateway execution role 权限。*
...
*图 10：在 Quick Desktop 中启用场景一连接器。*

输入以下请求，检查 Quick 是否先查询订单需求，再调用库存工具。

> 请查询订单 ORDER-1001 的需求，并检查库存是否足够，说明缺货风险和缺货数量。

下图展示了一次业务调用示例。

![场景一的订单与库存工具调用示例](image/image-20260911130824379.png)

*图 11：场景一的订单与库存工具调用示例。*
...
订单 REST API 的路由使用 `AuthorizationType=NONE`，由 Gateway credential provider 注入 `Authorization: Basic ...`。订单函数必须先校验用户名和密码，再执行查询。

库存链路使用 Entra ID app-only access token，无需为库存 API 另建 Cognito 用户池。授权分两步完成：
...
*图 12：库存资源 API 的 access token 版本配置。*
...
*图 15：为库存客户端添加 InventoryReader 应用程序权限。*
...
| `EntraTenantId` | 库存 token 的签发 tenant ID；使用指定 tenant，不能填 `common` 或 `organizations` |
...
![场景二的库存 HTTP API 授权配置](image/image-20260914140721956.png)

*图 20：场景二的库存 HTTP API 授权配置。*

![场景二的库存 HTTP API JWT authorizer](image/image-20260914140747166.png)

*图 21：场景二的库存 HTTP API JWT authorizer。*

stage variables 保存预期的 tenant、客户端和 app role。两个 API 的 Lambda 调用权限均限定到账号、API、阶段、方法和路径。

![场景二的库存 API stage variables](image/image-20260914140816255.png)

*图 22：场景二的库存 API stage variables。*

**credential provider 与 Gateway target。** 模板创建引用外部 Secrets Manager 凭据的 API 密钥 credential provider 和 `CustomOauth2` credential provider。库存 target 以 `CLIENT_CREDENTIALS` 请求 app-only access token。
...
连接器的配置方式与场景一相同，但所有字段均使用**场景二栈**的输出：`GatewayUrl`、`InboundClientId`、`InboundTokenEndpoint` 和 `InboundScope`。client secret 从场景二的 `InboundUserPoolId` 与 `InboundClientId` 对应 app client 读取。
...
flowchart LR
    ...
    G -->|TOKEN_EXCHANGE / OBO| OAPI[订单 HTTP API]
    G -->|TOKEN_EXCHANGE / OBO| IAPI[库存 HTTP API]
...
`Orders.Read` 和 `Inventory.Read` 是 delegated permissions，允许 Gateway 代表用户访问对应资源；`OrdersReader` 和 `InventoryReader` 是对应资源 API 定义并分配给用户的 app roles，表示该用户的业务读取资格。**scope 与 app role 分别校验，不能互相替代或跨资源复用。**

### 准备 Entra ID 应用与凭据

在同一个 Entra ID tenant 中完成四个应用注册。这些应用用于连接器到下游 API 的用户委托链，独立于既有的 Quick SSO 企业应用。
...
注册名为 `Quick Client` 的应用，在 **Authentication → Add a platform → Web** 中添加步骤 1 记录的 Redirect URL。本场景使用带 client secret 的 authorization code flow，因此选择 Web 平台，不启用 implicit grant。
...
*图 45：为 Quick Client 添加 mcp.invoke delegated permission。*
...
**两个下游 HTTP API。** 两者使用 Lambda proxy payload 2.0 和原生 JWT authorizer。issuer 相同，audience 分别对应各自的资源 API。

![场景三的订单 HTTP API JWT authorizer](image/image-20260914213208704.png)

*图 51：场景三的订单 HTTP API JWT authorizer。*

![场景三的库存 HTTP API JWT authorizer](image/image-20260914213237746.png)

*图 52：场景三的库存 HTTP API JWT authorizer。*

每条路由要求对应的 scope，Lambda 调用权限限定到对应 API 的阶段、方法和路径。

![场景三的订单 API route scope](image/image-20260914213348571.png)

*图 53：场景三的订单 API route scope。*

![场景三的库存 API route scope](image/image-20260914213406476.png)

*图 54：场景三的库存 API route scope。*

两个 stage 的 `authorizationMode` 均为 `ENTRA_DELEGATED_USER_ROLE`，`expectedAuthorizedParty` 均为 Gateway API Application ID。订单 stage 的 `requiredUserRole` 为 `OrdersReader`，库存 stage 的值为 `InventoryReader`。

![场景三的订单 API stage variables](image/image-20260914213453150.png)

*图 55：场景三的订单 API stage variables。*

![场景三的库存 API stage variables](image/image-20260914213505063.png)

*图 56：场景三的库存 API stage variables。*

**出站 OBO 配置。** Microsoft OAuth credential provider 引用外部 Secrets Manager client secret。两个 OpenAPI target 使用 `TOKEN_EXCHANGE`，分别请求 `api://<OrderApiApplicationId>/Orders.Read` 和 `api://<InventoryApiApplicationId>/Inventory.Read`。

![场景三的订单 OpenAPI target 与 OBO 配置](image/image-20260914214033066.png)

*图 57：场景三的订单 OpenAPI target 与 OBO 配置。*

![场景三的库存 OpenAPI target 与 OBO 配置](image/image-20260914214101672.png)

*图 58：场景三的库存 OpenAPI target 与 OBO 配置。*

**Gateway 入站配置。** 核对 Entra ID issuer、Gateway audience、`mcp.invoke` scope，以及 `tid` 和 `azp` claim 约束。

![场景三的 Gateway 入站 JWT 配置](image/image-20260914213707903.png)

*图 59：场景三的 Gateway 入站 JWT 配置。*
...
*图 60：将 Gateway URL 添加到 Gateway API 的 identifierUris。*
...
| Redirect URL | 前文注册到 Quick Client 的 Quick 回调地址 |

![场景三的连接器 MCP 端点配置](image/image-20260913140013411.png)

*图 61：场景三的连接器 MCP 端点配置。*

本示例沿用仓库 [Entra ID 配置指南](../gateway/Scenario3UserDelegation/ENTRA_SETUP.md) 中的 tenant 专属 v1.0 authorization/token endpoints，以处理 Quick 携带 `resource` 参数时的兼容性。Gateway API、Order API 和 Inventory API 仍设置 `api.requestedAccessTokenVersion=2`。**OAuth endpoint 版本与 access token 版本分别配置**，参见 https://learn.microsoft.com/en-us/entra/identity-platform/access-tokens

按 Quick 提示完成登录和用户授权。

![场景三的用户登录与授权示例（一）](image/image-20260913122906395.png)

*图 62：场景三的用户登录与授权示例（一）。*

![场景三的用户登录与授权示例（二）](image/image-20260913122950228.png)

*图 63：场景三的用户登录与授权示例（二）。*
...
*图 65：在 Quick Desktop 中启用场景三连接器。*
...
*图 66：场景三中已授权用户的业务调用示例。*

未分配业务角色的用户即使能够看到连接器并完成 **Sign in**，仍不能取得对应业务数据。若企业应用启用了 **Assignment required**，请求也可能在 Entra ID 授权或 OBO 阶段提前被拒绝；验证 Lambda 的角色校验分支时，需要区分拒绝发生的位置。

![场景三中未授权用户的连接器登录示例](image/image-20260914182828652.png)

*图 67：场景三中未授权用户的连接器登录示例。*

![场景三中未授权用户的访问拒绝示例](image/image-20260914183212957.png)

*图 68：场景三中未授权用户的访问拒绝示例。*

### 扩展到更细粒度的业务授权

如需按数据行或字段控制访问，业务函数可以从已经由 API Gateway 验证的 JWT claims 中读取 `tid` 和 `oid`，结合业务权限数据定位用户并实施授权。继续沿用可信 authorizer claims 作为身份来源，避免直接信任调用方提交的身份字段。
...
本场景保留 Cognito client credentials flow 作为入站认证方式。工具调用和 OAuth token 请求使用各自的网络路径：前者经过 Gateway interface VPC endpoint，后者访问公共 Cognito user-pool domain 的 `/oauth2/token`。
...
Quick 支持分别配置 resource-server VPC connection（`VpcConnectionArn`）和 auth-server VPC connection（`AuthVpcConnectionArn`），用于承载 MCP 流量和 OAuth 流量。如果授权服务器提供可从 VPC 访问的 OAuth endpoint，可以独立配置其 auth-server VPC connection。具体配置见https://docs.aws.amazon.com/quick/latest/userguide/mcp-integration.html。
...
**专属 VPC Lambda。** 两个业务函数配置到本场景的私有子网，使用专属的业务安全组、execution role 和日志组。它们由场景四栈管理，随栈创建和删除；场景一至三继续使用各自引用的共享业务函数。
...
*图 70：为 MCP 调用选择 VPC 连接，为 Cognito OAuth 保留 Public network。*

在 **Authenticate** 页面选择 **Service-to-service OAuth**，填写本场景的 Client ID、Client secret 和 Token URL，并核对 scope。

![场景四的连接器服务到服务 OAuth 配置](image/2026-09-15_162727_970.png)

*图 71：场景四的连接器服务到服务 OAuth 配置。*
...
*图 73：通过场景四连接器查询订单 ORDER-1003 并检查库存。*
